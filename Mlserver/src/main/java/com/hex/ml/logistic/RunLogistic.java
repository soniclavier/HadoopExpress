...
public final class RunLogistic {
	...
	// I am putting the length as hard coded now but we will take the lables
	...
	static JSONObject mainToOutput(String[] args, PrintWriter output)
			throws Exception {
		...
		if (parseArgs(args)) {
			...
			LogisticModelParameters lmp = LogisticModelParameters
					.loadFrom(new File(modelFile));
			...
			while (line != null) {
				Vector v = new SequentialAccessSparseVector(
						lmp.getNumFeatures());
				...
				confusionMatrix.set(target, classResult,
						confusionMatrix.get(target, classResult) + 1);
				line = in.readLine();
			}
			in.close();

			JSONArray confusionMat = new JSONArray();
			for (int i = 0; i < k; i++) {
				System.out.println();
				JSONArray row = new JSONArray();
				for (int j = 0; j < k ; j++) {
					System.out.print(confusion[i][j] + " ");
					row.put(confusion[i][j]);
				}
				confusionMat.put(row);
			}

			/*
			 * if (showAuc) { output.printf(Locale.ENGLISH, "AUC = %.2f\n",
			 * collector.auc()); } if (showConfusion) { Matrix m =
			 * collector.confusion(); output.printf(Locale.ENGLISH,
			 * "confusion: [[%.1f, %.1f], [%.1f, %.1f]]\n", m.get(0, 0),
			 * m.get(1, 0), m.get(0, 1), m.get(1, 1)); m = collector.entropy();
			 * output.printf(Locale.ENGLISH,
			 * "entropy: [[%.1f, %.1f], [%.1f, %.1f]]\n", m.get(0, 0), m.get(1,
			 * 0), m.get(0, 1), m.get(1, 1)); }
			 */
			if (showConfusion) {
				output.printf(
						Locale.ENGLISH,
						"confusion: [[%.1f, %.1f,%.1f], [%.1f, %.1f,%.1f], [%.1f, %.1f,%.1f]]\n",
						confusionMatrix.get(0, 0), confusionMatrix.get(0, 1),
						confusionMatrix.get(0, 2), confusionMatrix.get(1, 0),
						confusionMatrix.get(1, 1), confusionMatrix.get(1, 2),
						confusionMatrix.get(2, 0), confusionMatrix.get(2, 1),
						confusionMatrix.get(2, 2));
				output.printf(Locale.ENGLISH, "kappa:[%.5f]", kappa());
			}

			//hard coding category information
			result.append("Categories","Iris-setosa,Iris-versicolor,Iris-virginica");
			result.append("ConfusionMat", confusionMat);
			// result.append(confusionMatrix);
			result.append("Kappa", kappa());
			result.append("StandardDeviation", getNormalizedStats()
					.getStandardDeviation());
			result.append("Average", getNormalizedStats().getAverage());
		}
	 ...
	 * 
	 * double p = (0.5 + confusionMatrix.get(confusionMatrix.rowSize()-1,
	 * confusionMatrix.rowSize()-1)) / (1 + confusionMatrix.get(0, 0) +
	 * confusionMatrix.get(confusionMatrix.rowSize()-1,
	 * confusionMatrix.rowSize()-1)); for(int
	 * i=0;i<confusionMatrix.rowSize();i++){
	 * 
	 ...
	 * entropy.set(1, 1, confusionMatrix.get(1, 1) * Math.log(p)); } return
	 * entropy; }
	 */
